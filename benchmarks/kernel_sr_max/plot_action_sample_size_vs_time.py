...
@mx.config
def mx_config():
    action_sample_size = range(25, 275, 25)
...
def _save(_run, _log):
    ...
    if _run.main_function.__name__ == "main" and _run.result is not None:
        ...
        result[:] = _run.result
        np.savez("results/kernel_sr_max_action_sample_size_vs_time_data.npz", *result)


@mx.post_run_hook
def _plot(action_sample_size, _run, _log):
    """Plot the results."""

    if _run.main_function.__name__ == "main" and _run.result is not None:
        _log.info("Plotting the results.")
        data = np.load("results/kernel_sr_max_action_sample_size_vs_time_data.npz")

        fig = plt.figure()
        ax = plt.axes()
        plt.grid(zorder=0)

        x = np.array(list(action_sample_size))
        ...
        plt.savefig("results/kernel_sr_max_action_sample_size_vs_time.png")
